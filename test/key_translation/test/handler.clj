...
(deftest test-app
  (testing "the request does not exist yet"
    (let [response (app (request :get "/tenants/1003/events/010100120001"))]
      (is (= (:status response) 400))))

  (testing "add and retrieve a response"
    (let [

            request2 (assoc (request :post "/tenants/1003/events/23")
                                      :multipart-params    {"id"
                                                                         {:filename "vc.csv",
              ...
              response2 (app (request :get "/tenants/1003/events/010100120001" ))]
